	...
	ChainlinkClient
{
	...
	///@notice Mapping to keep track of request to token id
	...
	event ChainCompatibilityUpdated(
	...
	///@notice OracleRequestMade event is emitted when a request is made to the oracle for engagement data
	event OracleRequestMade(uint256 indexed tokenId, bytes32 indexed requestId);
	///@notice OracleSettingsUpdated event is emitted when the oracle settings are successfully updated
	...
	event NFTBound(uint256 indexed tokenId);
	...
	function claimRewards(uint256 tokenId) public whenNotPaused {
		...
		require(
			stakingToken.transfer(_msgSender(), stakedAmount + reward),
	...
	function calculateReward(
		...
		uint256 reward = adNft
			.engagementScore
			.mul(stakerShare)
			.div(adNft.totalStake)
			.mul(rewardRate);
		return reward;
	}
	...
	function _getNextTokenId() private view returns (uint256) {
		return stakingToken.totalSupply() + 1;
	}
	...
	function buyAINFT(
		...
		require(
			stakingToken.approve(address(router), fees),
			"Failed to approve stakingToken transfer"
		);
	...
	function updateChainCompatibility(
		uint64 chainSelector,
		bool isCompatible
	) external onlyOwner {
	...
	function moveToChain(
		...
		onlyRole(ADMIN_ROLE)
		whenNotPaused
		onlyCompatibleChain(destinationChainSelector)
	{
		// require(_exist(tokenId), "NFT does not exist");
	...
	function requestEngagementData(
		...
		Chainlink.Request memory request = buildChainlinkRequest(
			jobId,
			address(this),
			this.fulfillEngagementData.selector
		);
	 ...
	 */
	function fulfillEngagementData(
		bytes32 _requestId,
		uint256 _engagementScore // recordChainlinkFulfillment(_requestId)
	) external {
